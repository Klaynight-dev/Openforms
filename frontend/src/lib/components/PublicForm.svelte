<script lang="ts">
  ...
  import { api, ApiError, setEmbedToken, type SignedFileDescriptor } from "$api/client.ts";
  ...
  let {
    ...
    /** Slug public du formulaire à afficher. */
    slug: string;
    /** Rendu destiné à un iframe sur un site tiers : pas de fond plein écran,
     *  hauteur remontée à la page hôte, pas de formulaire de connexion. */
    embed?: boolean;
    /** Clé d'intégration (ofe_…) présentée à la place du cookie de session. */
    embedToken?: string | null;
    /** Adresse du formulaire sur l'instance, proposée quand l'iframe ne suffit pas. */
    standaloneUrl?: string;
  } = $props();

  /** Racine du rendu : sert à mesurer la hauteur à remonter à la page hôte. */
  ...
  // Intégré, l'avertissement s'afficherait à la navigation du site hôte, pour
  // un formulaire que le visiteur ne regarde peut-être même plus.
  $effect(() => {
    if (embed || !hasUnsavedInput) return;
  ...
  /**
   * Messages envoyés à la page hôte d'un formulaire intégré.
   * `targetOrigin: "*"` est volontaire : l'hôte est par définition inconnu du
   * formulaire, et rien de confidentiel ne transite ici- une hauteur en
   * pixels et un accusé d'envoi. Côté hôte, embed.js filtre sur la source.
   */
  function postToHost(message: Record<string, unknown>) {
    if (!embed || typeof window === "undefined" || window.parent === window) return;
    window.parent.postMessage({ ...message, slug }, "*");
  }

  // La page hôte ne peut pas mesurer le contenu d'un iframe d'une autre
  // origine : sans ce relais, le formulaire vit dans un cadre à hauteur fixe,
  // avec sa propre barre de défilement.
  $effect(() => {
    if (!embed || !root || typeof ResizeObserver === "undefined") return;
    const report = () => postToHost({ type: "openforms:resize", height: Math.ceil(root!.scrollHeight) });
  ...
  onMount(() => {
    setEmbedToken(embed ? embedToken : null);
    loadForm();
  ...
  /**
   * Ramène le visiteur en haut du formulaire. Intégré, la fenêtre appartient au
   * site hôte : on remonte le cadre lui-même, et on lui demande de le cadrer.
   */
  function scrollToTop() {
    if (embed) {
      root?.scrollIntoView({ behavior: "smooth", block: "start" });
      postToHost({ type: "openforms:scroll" });
      return;
    }
...
<!-- Intégré, le fond reste transparent et la hauteur libre : c'est la page
     hôte qui fournit la page, le formulaire n'est qu'un bloc dedans. -->
<div class={embed ? "py-4" : "min-h-screen bg-[color:var(--surface-bg)] py-8"}>
  <main class="mx-auto max-w-2xl px-4" bind:this={root}>
    {#if loading}
    ...
    {:else if statusError === 401 && embed}
      <!-- Un cadre tiers ne peut pas ouvrir de session : le cookie de
           l'instance y serait bloqué comme cookie tiers. Pour un formulaire
           réservé, l'intégration passe par une clé d'embed ; à défaut, on
           renvoie le visiteur vers le formulaire sur l'instance. -->
      <div class="gform-card p-8 text-center">
        ...
        <h2 class="text-lg font-bold mb-2">Formulaire réservé</h2>
        <p class="text-sm text-[color:var(--muted)] mb-4">
          Ce formulaire demande une connexion, impossible depuis un site externe.
        </p>
        {#if standaloneUrl}
          <a class="btn-primary inline-flex items-center gap-2" href={standaloneUrl} target="_blank" rel="noopener">
            <IconLink size={16} /> Ouvrir le formulaire
          </a>
        {/if}
      </div>
