...
export const formController = new Elysia({ prefix: "/api/v1/forms" })
  ...
  .get(
    ...
    async ({ params, set, auth, apiKey, request }) => {
      ...
      // visiteur anonyme. Sans cette mise à l'écart, l'identité qu'elle porte
      // satisferait le « il faut être connecté » de n'importe quel formulaire
      // PRIVATE- une clé publiée sur un site tiers les ouvrirait tous.
